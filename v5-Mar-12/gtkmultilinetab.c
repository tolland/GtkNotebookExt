...
gtk_multiline_tab_expose (GtkWidget *widget, GdkEventExpose *event)
{
    ...
    for (i = 0; children; children = children->next, i++)
    {
        ...
        if (state_type == GTK_STATE_ACTIVE)
        {
            ...
            color = gtk_widget_get_style(GTK_WIDGET (widget))->light[GTK_STATE_ACTIVE];
            gdk_cairo_set_source_color (cr, &color);

            gdk_cairo_rectangle (cr, &allocation);
            cairo_fill (cr);

            color = gtk_widget_get_style(GTK_WIDGET (widget))->dark[GTK_STATE_ACTIVE];
            gdk_cairo_set_source_color (cr, &color);
            cairo_set_line_width (cr, 1);

            cairo_move_to (cr, allocation.x, allocation.y + allocation.height);
